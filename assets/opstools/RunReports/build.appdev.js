...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/RunReports
        ...
        async.series([
            ...
            function (next) {
                ...
                AD.log('<green>building</green> opstools/RunReports JS files');

                // Minify js/ejs files
                transform({
                    main: path.join('opstools', 'RunReports', 'RunReports'),
                    config: "stealconfig.js"
                }, {
                        minify: true,
                        ignore: [
                            /^.*(.css)+/, // Ignore css files
                            /^(?!opstools\/RunReports.*)/, // Ignore all are not plugin scripts
                        ]
                    }).then(function (transform) {
                        // Get the main module and it's dependencies as a string
                        var main = transform();

                        fs.writeFile(path.join('opstools', 'RunReports', 'production.js'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/RunReports JS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/RunReports JS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function (next) {
                AD.log('<green>building</green> opstools/RunReports CSS files');
                ...
                transform({
                    ...
                    config: "stealconfig.js"
                        ...
                        fs.writeFile(path.join('opstools', 'RunReports', 'production.css'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/RunReports CSS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/RunReports CSS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'RunReports', 'production.js'), tag:'packages/OpsPortal-RunReports.js', replace:'OpsPortal/production.js'}
                ];
