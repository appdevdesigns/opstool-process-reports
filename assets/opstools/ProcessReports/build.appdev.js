...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/ProcessReports
        ...
        async.series([
            ...
            function (next) {
                ...
                AD.log('<green>building</green> opstools/ProcessReports JS files');

                // Minify js/ejs files
                transform({
                    main: path.join('opstools', 'ProcessReports', 'ProcessReports'),
                    config: "stealconfig.js"
                }, {
                        minify: true,
                        ignore: [
                            /^.*(.css)+/, // Ignore css files
                            /^(?!opstools\/ProcessReports.*)/, // Ignore all are not plugin scripts
                        ]
                    }).then(function (transform) {
                        // Get the main module and it's dependencies as a string
                        var main = transform();

                        fs.writeFile(path.join('opstools', 'ProcessReports', 'production.js'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/ProcessReports JS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/ProcessReports JS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function (next) {
                AD.log('<green>building</green> opstools/ProcessReports CSS files');
                ...
                transform({
                    ...
                    config: "stealconfig.js"
                        ...
                        fs.writeFile(path.join('opstools', 'ProcessReports', 'production.css'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/ProcessReports CSS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/ProcessReports CSS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'ProcessReports', 'production.js'), tag:'packages/OpsPortal-ProcessReports.js', replace:'OpsPortal/production.js'}
                ];
